...
const emit = defineEmits(['update:isDialogVisible', 'material-enabled'])
...
const enableMaterial = async () => {
  ...
  try {
    ...
    success.value = "Material activado correctamente"
    emit('material-enabled')
    ...
    console.error('Error al activar material:', error)
    if (error.response?._data?.backendMessage) {
      error_exists.value = error.response._data.backendMessage
    } else if (error.response?.status === 403) {
      error_exists.value = "No tiene permisos para activar materiales"
    } else {
      error_exists.value = "Hubo un problema al activar el material. Por favor, intente nuevamente."
    }
  } finally {
    loading.value = false
  }
  ...
  >
    <template v-if="material">
      <VCard title="¿Activar material?">
        ...
        />
        <VCardText>
          ¿Estás seguro que deseas activar el material <b>{{ material }}</b>?<br>
          <VAlert
            v-if="error_exists"
            type="error"
            class="mt-3"
